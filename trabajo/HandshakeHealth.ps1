...
function HandshakeTable{
    ...
    $result = $result + '<table cellpadding="10" cellspacing="0" border="1" style="border-collapse: collapse; width: 100%; font-family: Arial, sans-serif;">'
    ...
    for ($i = 0; $i -lt $Handshakes.Length; $i++) {
        ...
        $result = $result + '<td>' + $Triggers[$i].ToString() +'</td>'
        $result = $result + '<td>' + $okCount[$i].ToString() + '</td>'
        $result = $result + '<td>' + ($Triggers[$i] - $okCount[$i]).ToString() + '</td>'
...
$mail.To = 'dehanke@suppilers.tenaris.com'
...
$mail.HTMLBody = "<html>$head<body><h2>HOLA</h2><br>$HandshakeTable</body><html>"
$mail.Display()